...
def clf_proba_pipeline(models, X_train, X_test, y_train, y_test):
    '''Pipeline for propability predictions of dict of classifiers - models'''
...
def threshhold_clf_controller(models, proba_arr, y_test):
    '''Implementation of accuracy scores for classifiers - models in a form of array.
    	Predictions are also treated as array of 100 variants depending on uniform thresholds distribution'''
...
def show_ten_imgs(imgs, labels, title):
    fig, axes = plt.subplots(2, 5, figsize=(30, 10))
    for i, ax in enumerate(axes.flatten()):
        ax.imshow(np.transpose(imgs[i].numpy(), (1, 2, 0)))
        ax.set_title((lambda x: 'Male' if x else 'Female')(labels[i].numpy()))
